...
def plotDateHist(df):
    df[['Month', 'Year']] = df['Date'].apply(
        lambda x: pd.Series(prep.splitDate(x)))
...
def plotUniqueValues(df):
    ...
    colors = plt.cm.get_cmap('RdYlGn_r')(
        unique_value_counts / max(unique_value_counts))
    ...
    bars = plt.bar(unique_value_counts.index,
                   unique_value_counts, color=colors)
